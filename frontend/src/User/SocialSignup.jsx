...
function SocialSignup({ onSuccess, googleButtonText = "Sign up with Google", fbButtonText = "Sign up with Facebook" }) {
  ...
  /*useEffect(() => {
    if (!window.FB) {
      window.fbAsyncInit = function () {
        window.FB.init({
          appId: "1479204356854374", // <-- Replace with your Facebook App ID
          cookie: true,
          xfbml: true,
          version: "v19.0",
        });
  ...
  const handleGoogleSuccess = (credentialResponse) => {
    // Send credentialResponse to backend /social-signup
      ...
      window.FB.login(function (response) {
        if (response.authResponse) {
          window.FB.api('/me', { fields: 'name,email,picture' }, function (userInfo) {
            // Send userInfo to backend /social-signup
            onSuccess && onSuccess({
